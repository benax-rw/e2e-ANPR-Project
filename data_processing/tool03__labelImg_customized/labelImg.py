...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        if self.label_hist:
            ...
            print("No predifined classes found in /data/predefined_classes.txt (optional)")
    ...
    def show_bounding_box_from_annotation_file(self, file_path):
        if not file_path:
            print("Error: file_path is None or invalid.")
            return
        ...
        # Annotation file priority: YOLO > PascalXML > CreateML
        if os.path.isfile(txt_path):  # YOLO annotation
            self.load_yolo_txt_by_filename(txt_path)
        elif os.path.isfile(xml_path):  # Pascal VOC annotation
            self.load_pascal_xml_by_filename(xml_path)
        elif os.path.isfile(json_path):  # CreateML annotation
            self.load_create_ml_json_by_filename(json_path, file_path)
        else:
            print(f"Warning: No annotation file found for {file_path}")
